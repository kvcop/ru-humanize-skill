...
### 8. Антитеза «не X, а Y» — главный машинный штамп

Отрицание-противопоставление модель усвоила как признак глубины: сначала
отвергнуть очевидное, потом подставить «настоящее». Приём дешёвый и очень
заметный. В английских текстах его успели посчитать: «it's not just X, it's
Y» в годовых отчётах Fortune 500 вырос с 50 употреблений в 2023 году до
двух с лишним сотен в 2025-м. По-русски он вылезает в двух жанрах.

**Постановка цели в рабочих документах.** Не начинать цель с «нам нужен не
X, а Y», «это не просто X, а Y», «не очередной X». Такая фраза спорит с
невидимым собеседником и тащит в документ лишний шум. Если документ
...
**Афористичная антитеза в презентациях, заголовках и слоганах.** Фраза
звучит эффектно и почти ничего не сообщает: половину предложения занимает
то, чего автор не имел в виду. Переписывать прямым утверждением. Иногда
предложение проще вычеркнуть — соседняя фраза обычно уже несёт этот смысл.

- ❌ Опасна не выдумка, опасна полуправда.
- ✅ Самая дорогая ошибка — полуправда.
- ❌ Неприятна не ошибка, а её невидимость.
- ✅ Хуже всего — невидимость ошибки.
- ❌ Выигрывает не машина вместо человека, а человек с машиной.
- ✅ Выигрывает человек с машиной.

Формы, которые надо ловить: «не X, а Y»; «X — это не Y, это Z»; «это не
просто X»; «дело не в X, дело в Y»; «не столько X, сколько Y»; хиазм с
повтором глагола («опасна не выдумка, опасна полуправда»).
...
### 12. Имитация живой речи вместо живой речи

Разговорность модель изображает набором приёмов, которые у человека
встречаются поштучно, а у неё идут подряд.

- Цепочки рубленых односоставных: «Коротко. Точно. По делу.» Один такой ряд
  на текст читается как приём, три — как тик. Разворачивать в обычное
  предложение.
- Риторический вопрос с немедленным самоответом: «Что это значит на
  практике? А вот что.» Вопрос убрать, ответ оставить.
- Псевдотерапевтические поддакивания: «и это нормально», «ты не
  ошибаешься», «звучит знакомо?». Вычёркивать целиком.
- Эмодзи-буллеты (⚡, 🎯, 💡) в списке, который и так структурирован.
- Заход от общего: «в современном мире», «не секрет, что», «в эпоху
  цифровизации». Начинать с первого содержательного слова.

Живая речь держится на другом: короткое слово вместо длинного, прямой
глагол, конкретный пример, изредка идиома («гладко было на бумаге») —
модели её обычно не достают.

### 13. Деепричастный хвост вместо мысли

Модель добивает предложение оборотом, который выглядит как вывод, а несёт
ноль информации: «подчёркивая важность», «тем самым демонстрируя»,
«что отражает общую тенденцию», «открывая новые возможности». Хвост
цепляется к любому факту и с ним же выбрасывается.

- ❌ Команда перешла на vLLM, что подчёркивает важность оптимизации инференса.
- ✅ Команда перешла на vLLM. Латентность упала с 800 до 210 мс.
- ❌ Логи собираются в одном месте, тем самым упрощая диагностику.
- ✅ Логи собираются в одном месте — диагностика идёт по одному файлу.

Проверка простая: убрать хвост. Если предложение ничего не потеряло, хвост
был украшением. Если потеряло — переписать хвост в самостоятельное
предложение с конкретикой.

### 14. Синонимическая чехарда

Модель боится повторить слово и на каждом шаге подставляет синоним:
«нейросеть» → «модель» → «искусственный интеллект» → «алгоритм» → «система»
в одном абзаце про одну и ту же штуку. Читатель начинает искать разницу
между ними, потому что в техническом тексте разные слова обычно означают
разные вещи.

- ❌ Модель обучалась неделю. Нейросеть показала рост качества, а алгоритм
  при этом не потерял скорость.
- ✅ Модель обучалась неделю: качество выросло, скорость не просела.

Ключевой термин выбрать один раз и повторять. Синоним подставлять только
когда он действительно называет другой объект.

### 15. Пунктуационные отпечатки

Расстановка знаков выдаёт генерацию не хуже лексики.

- Лишняя запятая после короткого обстоятельства в начале: ❌ «Благодаря
  новой схеме, мы сократили время» → ✅ «Благодаря новой схеме мы сократили
  время».
- Обособление «однако» в начале предложения: ❌ «Однако, схема не
  завелась» → ✅ «Однако схема не завелась».
- «такие как» вместо двоеточия: ❌ «Нужны инструменты, такие как профайлер
  и трассировка» → ✅ «Нужны инструменты: профайлер и трассировка».
- Отсутствие скобок и многоточий. Модель их почти не ставит, потому что не
  уточняет и не делает пауз. Живой текст ими пользуется — скобка для
  оговорки, многоточие для оборванной мысли.

### 16. Следы генерации в артефакте

Перед отправкой вычищать всё, что выдаёт способ производства текста.

- Плейсхолдеры и рыба: `[вставьте название]`, «TODO: дополнить», «Ваш
  текст здесь».
- Оговорки про обучение: «по состоянию на мою дату обучения», «у меня нет
  доступа к актуальным данным». Если данных нет, писать прямо: «данные за
  июль не проверял».
- Обращение к читателю от лица правщика: «я обновил документ», «надеюсь,
  так понятнее» внутри самого документа.
- Разметка не той системы: markdown в Jira или Confluence, где нужна wiki-
  разметка; артефакты цитирования вроде `oaicite` или `[cite: 1]`.
...
- **Честная концовка**: конкретный вывод, открытый вопрос или наблюдение, которое читатель унесёт с собой. Формулу «challenges / bright future» вычёркивать.
...
14. Есть антитеза «не X, а Y» / «X — это не Y, это Z» / «дело не в X, дело в Y»? — переписать прямым утверждением («Цель — Y», «Хуже всего — Y») или вычеркнуть предложение.
...
16. Разговорность изображена приёмами — цепочка рубленых фраз, риторический вопрос с самоответом, «и это нормально», эмодзи-буллеты, заход «в современном мире»?
17. Деепричастные хвосты-пустышки («подчёркивая важность», «тем самым демонстрируя», «что отражает тенденцию»)? — убрать или развернуть в конкретику.
18. Один и тот же объект назван тремя разными словами («модель», «нейросеть», «алгоритм»)? — оставить один термин.
19. «Однако,» в начале предложения, запятая после короткого обстоятельства, «такие как» вместо двоеточия?
20. Остались следы генерации — плейсхолдеры, оговорки про дату обучения, «я обновил документ», markdown в Jira/Confluence?
